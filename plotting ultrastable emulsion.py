...
times = [int(10000*1),int(10000*20),int(10000*100)]

# plot for each square
for i,n in enumerate(times):

    phi = np.loadtxt(f"emulsion-no-surfactant-small-vrs-data/phi{n}.txt")
    ...
    phi = np.loadtxt(f"emulsion-surfactant-small-vrs-data/phi{n}.txt")
    px =  np.loadtxt(f"emulsion-surfactant-small-vrs-data/px{n}.txt")
    px = px[cs]
    py =  np.loadtxt(f"emulsion-surfactant-small-vrs-data/py{n}.txt")
    py = py[cs]
    print(np.sum(phi[cs]*dx*dx))
...
fig.text(0.02, 0.70, 'Time $t = 10$', va='center', rotation='vertical', fontsize=12)
fig.text(0.02, 0.45, 'Time $t = 200$', va='center', rotation='vertical', fontsize=12)
fig.text(0.02, 0.19, 'Time $t = 1000$', va='center', rotation='vertical', fontsize=12)
...
# c = np.loadtxt(f"emulsion-surfactant-small-vrs-data/c{n}.txt")
