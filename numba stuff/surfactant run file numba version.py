...
@njit(parallel = True)
def calc_mu_phi(c_p,phi,mu_phi,cs,beta,kappa,xl,dx,sim_type):
    #calculate mu_phi
...
@njit(parallel = True)
def calc_omega_D(omega,D_x,D_y,du_x,du_y):
    # calculate omega
...
@njit       
def calc_f(f,dh_dxs,dh_dys,c_h,dmu_phi,mu_phi,phi,c,dc,dmu_c,p,p_dh,dsigma,sigma_xx,sigma_xy,sigma_yx,sigma_yy,css,cs,rcss,dx,B,kBT,sim_type):
     # calculate sigma
    p_ch = p*c_h
...
@njit(parallel = True)
def update_phi(phi,uu,dphi,mu_phi,cs,css,dt,dx,M,sim_type):
    phi[cs] += dt*(-dot(uu[css],dphi)+ M*lap(mu_phi,dx)) 
    BC_standard(phi,sim_type)

@njit(parallel = True)
def update_c(c,uu,dc,J_c,cs,css,kBT,gamma_t,dx,dt,sim_type):
    c[cs] += dt*(-dot(uu[css],dc) + (kBT/gamma_t)*lap(c,dx) + div(J_c[0],J_c[1],dx)) 
    BC_standard(c,sim_type)  

@njit(parallel = True)
def update_p(dp_x,dp_y,u_dp,p_D,p,uu,D_x,D_y,h,omega,css,rcss,dx,dt,gamma_r,B,sim_type):
    # update p
    dp_x[0] = dev_x(p[0],dx)
    dp_x[1] = dev_y(p[0],dx)
    dp_y[0] = dev_x(p[1],dx)
...
@njit(parallel = True)
def update_u_ns(u_du,f,lap_u,du_x,du_y,uu,u_star,prs,dev_prs,dx,dt,eta,rho,cs,css,sim_type):
...
def update_batch_1(f,c_h,dh_dxs,dh_dys,dmu_phi,dc,p_dh,c_p,dsigma,sigma_xx,sigma_xy,sigma_yx,sigma_yy,c,p,phi,mu_phi,mu_c,dmu_c,h,dphi,omega,D_x,D_y,du_x,du_y,cs,css,rcss,B,kBT,xl,beta,kappa,dx,sim_type):
    c_p[css] = c[cs]*p[css]
    BC_standard(c_p[0],sim_type)
    BC_standard(c_p[1],sim_type)
    calc_mu_phi(c_p,phi,mu_phi,cs,beta,kappa,xl,dx,sim_type)
    #######################################################################################################################################################
    calc_mu_c(mu_c,p,dphi,cs,css,kBT,xl,sim_type)
    dmu_c[0],dmu_c[1] = dev_x(mu_c,dx),dev_y(mu_c,dx)
    #######################################################################################################################################################
    calc_h(h,p,dphi,css,kBT,xl,sim_type)
    #######################################################################################################################################################
    calc_omega_D(omega,D_x,D_y,du_x,du_y)
    #######################################################################################################################################################
    c_h[css] = c[cs] * h[css]
    BC_standard(c_h[0],sim_type)
    BC_standard(c_h[1],sim_type)
        
    calc_f(f,dh_dxs,dh_dys,c_h,dmu_phi,mu_phi,phi,c,dc,dmu_c,p,p_dh,dsigma,sigma_xx,sigma_xy,sigma_yx,sigma_yy,css,cs,rcss,dx,B,kBT,sim_type)

def update_batch_2(f,u_du,lap_u,du_x,du_y,u_star,prs,dev_prs,h,phi,uu,c,p,mu_phi,dmu_c,J_c,dphi,dc,dp_x,dp_y,u_dp,p_D,D_x,D_y,omega,cs,css,rcss,dt,dx,M,eta,rho,kBT,gamma_t,gamma_r,B,sim_type):
    # update phi
    update_phi(phi,uu,dphi,mu_phi,cs,css,dt,dx,M,sim_type)
    #######################################################################################################################################################
    # update c
    ...
    update_c(c,uu,dc,J_c,cs,css,kBT,gamma_t,dx,dt,sim_type)
    #######################################################################################################################################################
    update_p(dp_x,dp_y,u_dp,p_D,p,uu,D_x,D_y,h,omega,css,rcss,dx,dt,gamma_r,B,sim_type)
    #######################################################################################################################################################
    update_u_ns(u_du,f,lap_u,du_x,du_y,uu,u_star,prs,dev_prs,dx,dt,eta,rho,cs,css,sim_type)
...
def main(dt = 0.01, dx = 0.5, Nx = 512, Ny = 256, Nt = int(1e6), 
    ...
    for i in range(Nt + 1):
        ...
        #######################################################################################################################################################
        update_batch_1(f,c_h,dh_dxs,dh_dys,dmu_phi,dc,p_dh,c_p,dsigma,sigma_xx,sigma_xy,sigma_yx,sigma_yy,c,p,phi,mu_phi,mu_c,dmu_c,h,dphi,omega,D_x,D_y,du_x,du_y,cs,css,rcss,B,kBT,xl,beta,kappa,dx,sim_type)
        #######################################################################################################################################################
        #######################################################################################################################################################
        # UPDATE ALL FIELDS + APPLY APPROPIATE BC'S
        #######################################################################################################################################################
        update_batch_2(f,u_du,lap_u,du_x,du_y,u_star,prs,dev_prs,h,phi,uu,c,p,mu_phi,dmu_c,J_c,dphi,dc,dp_x,dp_y,u_dp,p_D,D_x,D_y,omega,cs,css,rcss,dt,dx,M,eta,rho,kBT,gamma_t,gamma_r,B,sim_type)
        #######################################################################################################################################################
...
main(name="parallel speed test",Nx=500,Ny=500,dt=0.001,dx=1.0,Nt=int(1e4),sim_type=1)
